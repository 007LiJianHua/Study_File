...
>   * <font color="red">**注意：多个表中联结查询时，先去找到能连接另外表的表,利用column**</font>
> * 语法:
...
mysql> select students.SID,students.Name,courses.CID,courses.Cname ,scores.Score 
	   from students left join scores on students.SID = scores.SID left join courses on scores.CID = courses.CID;
+-----+--------------+------+-----------------+-------+
